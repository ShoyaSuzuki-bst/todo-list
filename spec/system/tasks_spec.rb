  ...
  describe '/tasks/index' do
    ...
    context 'access /tasks/index' do
      ...
      it 'is expected to sort tasks descending order of created_at' do
        ...
        # string array
        created_at_datetimes = all('div.task-row div.task-created_at').map{ |i| i.text }
        # 「作成日の新しい順」となっているはずのdateと、DBから参照したデータをソートしたもので比較する。
        expect(created_at_datetimes).to eq(valid_created_at_array)
      end

      it 'is expected to sort tasks descending order of limited_at' do
        visit "/tasks/sort/limited_at"
        display_limited_at_datetimes = all('div.task-row div.task-created_at').map{ |i| i.text }
        expect(display_limited_at_datetimes).to eq(valid_limited_at_array)
      end
    end
  end
end
